...
The application follows a modern Jamstack architecture, with a decoupled frontend and backend, deployed to specialized cloud providers.

-   **Frontend (React App on Vercel):**
    -   **Hosting Provider:** Vercel.
    -   **Deployment Process:** The frontend is a static site generated by Vite. The `npm run build` command compiles the React/TypeScript source into optimized HTML, CSS, and JavaScript assets in the `frontend/dist` directory. This `dist` directory is deployed to Vercel.
    -   **Environment Variables:** Vercel's project settings are used to securely store and expose the necessary environment variables to the frontend:
        -   `VITE_SUPABASE_URL`: The public URL for the Supabase project.
        -   `VITE_SUPABASE_ANON_KEY`: The public anonymous key for the Supabase project.

-   **Backend (Supabase Edge Functions):**
    -   **Hosting Provider:** Supabase.
    -   **Deployment Process:** The backend logic is encapsulated in Deno-based Edge Functions. They are deployed directly from the local development environment to the Supabase cloud using the Supabase CLI command: `supabase functions deploy <function_name>`.
    -   **Environment Variables & Secrets:** Sensitive keys required by the functions are configured as secrets within the Supabase project dashboard (`Settings > Edge Functions`):
        -   `ELEVENLABS_API_KEY`: The API key for the ElevenLabs text-to-speech service.
        -   `SUPABASE_SERVICE_ROLE_KEY`: The service role key for performing privileged operations on the Supabase backend.

-   **CI/CD (Future Work):**
    -   A continuous integration and deployment pipeline will be set up using GitHub Actions. This will automate linting, testing, building, and deploying both the frontend and backend upon pushes or merges to the `main` branch.
